...
async function main() {
  ...
  console.log("Deploying BusinessTravelBooking to Sepolia...");
  console.log(`Deployer: ${deployer}`);

  const { viem } = await network.create("sepolia");
  const publicClient = await viem.getPublicClient();
  const { contract, deploymentTransaction } =
    await viem.sendDeploymentTransaction("BusinessTravelBooking");
  ...
  deployment.contracts.businessTravelBooking = {
  ...
  console.log(`BusinessTravelBooking deployed at: ${contract.address}`);
