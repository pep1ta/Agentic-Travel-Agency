...
async function main() {
  ...
  console.log("Deploying BusinessTravelPolicy to Sepolia...");
  console.log(`Deployer: ${deployer}`);

  const { viem } = await network.create("sepolia");
  const publicClient = await viem.getPublicClient();
  const { contract, deploymentTransaction } =
    await viem.sendDeploymentTransaction("BusinessTravelPolicy");
  ...
  deployment.contracts.businessTravelPolicy = {
  ...
  console.log(`BusinessTravelPolicy deployed at: ${contract.address}`);
